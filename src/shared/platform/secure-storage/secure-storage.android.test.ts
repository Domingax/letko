...
describe('SecureStorageAdapter (android)', () => {
  ...
  it('get returns err without leaking key in error message', async () => {
    const { SecureStorage } = await import('@aparajita/capacitor-secure-storage')
    vi.mocked(SecureStorage.get).mockRejectedValue(new Error('keystore error'))
  ...
  it('set returns err without leaking value in error message', async () => {
    const { SecureStorage } = await import('@aparajita/capacitor-secure-storage')
    vi.mocked(SecureStorage.set).mockRejectedValue(new Error('keystore error'))
  ...
  it('remove returns err without leaking key in error message', async () => {
    const { SecureStorage } = await import('@aparajita/capacitor-secure-storage')
    vi.mocked(SecureStorage.remove).mockRejectedValue(new Error('keystore error'))
